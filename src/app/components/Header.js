    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 sm:px-12 flex items-center justify-between">
          ...
          >
            <span
              className={`block w-6 h-0.5 bg-slate-900 transition-all duration-300 ${menuOpen ? "rotate-45 translate-y-2" : ""
                }`}
            />
            <span
              className={`block w-6 h-0.5 bg-slate-900 transition-all duration-300 ${menuOpen ? "opacity-0" : ""
                }`}
            />
            <span
              className={`block w-6 h-0.5 bg-slate-900 transition-all duration-300 ${menuOpen ? "-rotate-45 -translate-y-2" : ""
      ...
      <nav
        className={`fixed top-[72px] left-0 right-0 z-40 px-6 py-6 flex flex-col gap-4 bg-white/95 backdrop-blur-xl border-b border-slate-200 shadow-xl transition-all duration-300 ${menuOpen
        ...
        {navLinks.map((link) => {
          ...
          return (
            <Link
              ...
              className={`text-base font-sans transition-colors ${isActive
